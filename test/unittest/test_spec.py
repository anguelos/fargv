...
def test_init_config_does_not_persist_stale_variadic_default():
    """init_config_if_missing must not allow a stale config to override a
    subsequently changed FargvVariadic coded default.
    Added: 2026-04-10, initiated by: Anguelos, reported by: nprenet.
    Regression: config written on first run with FargvVariadic([]) persisted
    as [] and overrode a later coded default of ['b','a'] on subsequent runs.
    """
    import tempfile, os
    from fargv.parameters import FargvVariadic
    ...
    try:
        ...
        ns, _ = fargv.parse({'items': FargvVariadic(['b', 'a'])},
        ...
        fargv.parse({'items': FargvVariadic([])}, **opts)

        # Run 2 with config: default changed — stale config must not win
        ns, _ = fargv.parse({'items': FargvVariadic(['b', 'a'])}, **opts)
        assert ns.items == ['b', 'a']
    finally:
